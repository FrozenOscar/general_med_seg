...
class Unet_decoder(nn.Module):
    def __init__(self, in_channels, cat_channels, out_channels,
        ...
        """
        Args:
            in_channels (List[int]): 上一层特征图的通道数
            cat_channels (int): cat之后的通道数
            out_channels (int): 当前decoder层输出特征图通道数
            enc_num (int): 需要下采样的编码器跳跃连接数
            center_num (int): 同层编码器跳跃连接数（常为1）
            dec_num (int): 需要上采样的解码器跳跃连接数
        """
...
if __name__ == '__main__':
    ...
    model = Unet_decoder([*channels], cat_channels=cat_channels,
